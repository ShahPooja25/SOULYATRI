...
const Chatbot: React.FC<ChatbotProps> = ({ onClose }) => {
  ...
  const userId = "test_user_1"; // Replace after login is integrated
  ...
  const sendToAI = async (text: string) => {
    ...
    try {
      const res = await axios.post<AIResponse>("http://localhost:5000/ask-ai", {
        userMessage: text,
        userName: name,
      });

      const reply = res.data.reply;
      const botReply: Message = { sender: "bot", text: reply };
    ...
    <div className="chatbot-container">
      ...
      {!submitted ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <p>
            <b>🧘 Hi! I’m YatriBot, your Soul Yatri assistant.</b>
            <br />
        ...
        <>
          ...
          {showOptions ? (
            ...
            >
              <button
                onClick={() =>
                  handleOptionClick("What to know about Soul Yatri")
                }
              >
              ...
              <button onClick={() => handleOptionClick("Our Services")}>
                Our Services
              </button>
